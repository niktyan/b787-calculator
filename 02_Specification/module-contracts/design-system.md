...
- Props: `value`, `options`, `onChange(next)`,
  ...
  по высоте совпадает с соседними SegmentedControl.
- Label-шрифт (closed field + option rows) — `body`-вариант с
  override `fontSize` = `heading3` (18 pt) в обоих size-классах
  (post-G2 readability feedback): выбранное состояние ВПП читается
  с одного взгляда, одинаково в Takeoff и Landing.

...
| `envelopeBarOutOfLookup` | `rgba(229,72,77,0.2)` (CrosswindChart operational-envelope dashed verticals — Polish-3. Identical in both themes; distinct from `dangerSoft` (10–12%) used for inline error tints. Token name kept for backward compatibility; the Polish-2 EnvelopePositionBar that originated it has been removed.) |
...
| `envelopeBarOutOfLookup` | `rgba(229,72,77,0.2)` (identical to dark theme; used by CrosswindChart per the dark-palette row above) |

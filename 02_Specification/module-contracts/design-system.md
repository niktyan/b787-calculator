...
#### Overlays
- `BottomSheet` — модальный slide-up sheet с backdrop и drag-handle.
  Используется в Settings → Language / Theme picker-ах.
- `BottomSheetOption` — option-row внутри `<BottomSheet>` с
  selected-индикатором (✓).

#### Settings/About list rows
- `NavigableSettingsRow` — label + value + chevron, whole row
  tappable. Открывает sub-screen или bottom-sheet.
...
- `InfoSettingsRow` — label + value pair, read-only (без chevron,
  без Pressable). Используется для MVP-permanent rows (Weight /
  Wind units в Settings).

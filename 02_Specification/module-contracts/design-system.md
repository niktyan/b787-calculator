...
| `envelopeBarBoundary` | `rgba(255,176,32,0.2)` (EnvelopePositionBar boundary zone — CG inside lookup envelope but outside operational. Identical in both themes; distinct from `warnSoft` (8%) used for disclaimer card backgrounds) |
| `envelopeBarOutOfLookup` | `rgba(229,72,77,0.2)` (EnvelopePositionBar out-of-lookup zone. Identical in both themes; distinct from `dangerSoft` (10–12%) used for inline error tints) |
...
| `envelopeBarBoundary` | `rgba(255,176,32,0.2)` (identical to dark theme; tested for legibility on `bgCard` surface) |
| `envelopeBarOutOfLookup` | `rgba(229,72,77,0.2)` (identical to dark theme; tested for legibility on `bgCard` surface) |
...
#### Inputs
- `NumericInput` — числовое поле ввода с keyboard `numeric-pad` или `decimal-pad`. Поддержка валидации, error state. На iOS поверх клавиатуры есть кнопка **Done** (`returnKeyType="done"`), её нажатие диспатчит `Keyboard.dismiss()` через `onSubmitEditing`. Для tap-outside-to-dismiss используется `KeyboardDismissView` (см. ниже).

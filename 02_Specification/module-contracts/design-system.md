...
#### Inputs
- `NumericInput` — числовое поле ввода. Всегда `keyboardType="decimal-pad"` + `inputMode="decimal"` + `autoCorrect`/`autoComplete`/`spellCheck` выключены + `textContentType="none"` + sanitizer в `onChangeText` (defence in depth от iPad ABC-режима, paste и сторонних клавиатур; код — `src/design-system/components/NumericInput/sanitizeDecimalInput.ts`). Field-область обёрнута в `Pressable` с `onPress → inputRef.focus()` — таргет всей видимой области поля, а не маленький хитбокс TextInput'а; `accessible={false}` на обёртке, чтобы VoiceOver продолжал ходить через сам TextInput. Error state, `returnKeyType="done"` + `onSubmitEditing → Keyboard.dismiss()`. Optional **`size?: 'compact' | 'regular'`** prop (default `compact`). `compact` = 44 pt minHeight, mono 16 pt value, `microUppercase` 9 pt label, `monoSmall` unit. `regular` = 64 pt minHeight, padding 14×20, `monoMedium` 24 pt value, `label` 12 pt label, `body` 16 pt unit — используется в iPad-regular Crosswind input column. **Reserved warning slot**: под полем всегда рендерится `<View>` с фиксированным `minHeight` (20 pt compact / 24 pt regular) под `testID={`${testID}-error-slot`}`. Когда `error` set, в слот помещается `<Text variant="caption" color="danger">`; иначе слот пустой. Это предотвращает layout-shift при появлении / исчезновении warning-а (см. 06-ui-spec.md § Экран 4 «Warning text reserved slot»).

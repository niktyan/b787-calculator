...
3. **Через bundled JSON-конфиг.** App Shell читает `src/core/modules/data.json` (rename из `coming-soon-modules` в Sprint 6 follow-up Block 4), передаёт через Context или prop в Main Menu screen. Никакой direct cross-module call.
...
6. **Обновить `src/core/modules/data.json`** — переключить `active: false` → `active: true` на entry-е модуля и добавить поле `route` (string, типа `/weight-balance`). Schema — discriminated union по `active`, см. `module-contracts/core.md` § `core/modules/`.

7. **Main Menu рендерит обновлённую карточку автоматически** — `useModules()` хук возвращает обе категории (active + coming-soon), Menu делит их по `active`-полю и переключает между `<ActiveModuleCard>` и `<ComingSoonCard>`. Никаких изменений в `menu.tsx` не требуется (см. § Экран 3 в 06-ui-spec).

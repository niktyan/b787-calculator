...
4. **Settings** — выбор языка (RU/EN), переключение темы (Auto/Light/Dark), выбор единиц измерения (Tons (t) для веса, KT для ветра в MVP; альтернативы Pounds / m/s видны как disabled-toggle с подсказкой «Available in upcoming release» — см. `06-ui-spec.md` § Экран 5).
...
**Важное архитектурное замечание про неактивные карточки.** Они являются **визуальными элементами Main Menu**, не отдельными модулями. Они не имеют собственного домена, данных или экранов в MVP — только содержимое в JSON-конфиге `src/core/coming-soon-modules/data.json`. Это позволяет: (а) безопасно показывать roadmap без риска навигации в нереализованный модуль, (б) обновлять roadmap без перевыпуска приложения через EAS Update в будущем, (в) превращать карточку из «coming soon» в активную одной правкой конфига при выкладке Phase 2.

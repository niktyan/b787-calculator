...
3. **Главное меню с roadmap-перспективой будущих модулей.** Одна активная карточка (Crosswind Landing) и **минимум три неактивные карточки-«тизеры»** будущих модулей: Crosswind Takeoff, Weight & Balance, Performance. Каждая «coming soon»-карточка содержит: иконку, название, краткое описание (1 предложение), бейдж с предполагаемой фазой («Phase 2», «Phase 3», «Phase 4»). Тап по неактивной карточке открывает короткий info-popup «This module is planned for an upcoming release. Stay tuned.» с кнопкой закрытия. Цель — показать пользователю, что приложение растёт, и создать ожидание расширения функциональности.
...
**Важное архитектурное замечание про неактивные карточки.** Они являются **визуальными элементами Main Menu**, не отдельными модулями. Они не имеют собственного домена, данных или экранов в MVP — только содержимое в JSON-конфиге `comingSoonModules.json`. Это позволяет: (а) безопасно показывать roadmap без риска навигации в нереализованный модуль, (б) обновлять roadmap без перевыпуска приложения через EAS Update в будущем, (в) превращать карточку из «coming soon» в активную одной правкой конфига при выкладке Phase 2.

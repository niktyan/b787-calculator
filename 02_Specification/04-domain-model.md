...
(RWYCC 5), `mediumToGood` (RWYCC 4), `medium` (RWYCC 3) и
`mediumToPoor` (RWYCC 2)**; одно оставшееся значение (`poor`)
валидно на уровне типов и UI (видимо как disabled-сегмент с
capability disclosure), но соответствующие lookup-данные
отсутствуют — выбор даёт
...
Type-only stubs для оставшихся двух (`ConstantParams`,
`NotAllowedParams`) живут в `src/features/crosswind/domain/strategy.ts`.
zod discriminated union в `data/schema.ts` перечисляет все 5 литералов;
для двух ещё-не-активных веток схема сейчас отвергает любой `params`,
что не пускает данные будущих стратегий в production до соответствующего PR.
...
содержит `byAircraft.b787_8.{dry, good, mediumToGood, medium, mediumToPoor}`.
...
| `byAircraft.<aircraft>.<condition>.strategyType` | `StrategyType` | Discriminator: одна из 5 strategy variants. В MVP активны `'bracketedLinear'` (Dry/Good/MediumToGood), `'variableSlopeBracketed'` (Medium) и `'cgOnlyPiecewise'` (MediumToPoor); остальные 2 литерала есть в union, но их `params` отклоняется (PR 7/8 их активируют). |
| `byAircraft.<aircraft>.<condition>.params` | object | Содержимое зависит от `strategyType`. Для `bracketedLinear`: `{ brackets[≥2] (crosswindKnots+intercept), slope, maxCap, decimals }`. Для `variableSlopeBracketed`: `{ brackets[≥2] (crosswindKnots+slope+intercept), maxCap, decimals }` — slope per bracket. Для `cgOnlyPiecewise`: `{ plateauValue, cgThreshold, slopeDivisor, decimals }` — no brackets, no maxCap. |

...
(RWYCC 5), `mediumToGood` (RWYCC 4) и `medium` (RWYCC 3)**;
остальные два значения (`mediumToPoor` / `poor`) валидны на уровне
типов и UI (видимы как disabled-сегменты с capability disclosure),
но соответствующие lookup-данные отсутствуют — выбор даёт
`DataNotAvailable.reason: 'condition-not-implemented'`. Активация
оставшихся двух — PR 6/7.
...
Type-only stubs для оставшихся трёх (`CGOnlyPiecewiseParams`,
`ConstantParams`, `NotAllowedParams`) живут в
`src/features/crosswind/domain/strategy.ts`. zod discriminated union в
`data/schema.ts` перечисляет все 5 литералов; для трёх ещё-не-активных
веток схема сейчас отвергает любой `params`, что не пускает данные
будущих стратегий в production до соответствующего PR.
...
| `byAircraft.<aircraft>.<condition>.strategyType` | `StrategyType` | Discriminator: одна из 5 strategy variants. В MVP активны `'bracketedLinear'` (Dry/Good/MediumToGood) и `'variableSlopeBracketed'` (Medium); остальные 3 литерала есть в union, но их `params` отклоняется (PR 6/7/8 их активируют). |
| `byAircraft.<aircraft>.<condition>.params` | object | Содержимое зависит от `strategyType`. Для `bracketedLinear`: `{ brackets[≥2] (crosswindKnots+intercept), slope, maxCap, decimals }`. Для `variableSlopeBracketed`: `{ brackets[≥2] (crosswindKnots+slope+intercept), maxCap, decimals }` — slope per bracket, не общий. |

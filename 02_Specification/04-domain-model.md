...
B787-8 operational limits (FCOM / Type Certificate values: weight
`[104.1, 227.93]` t — OEW-anchored minimum up to MTOW — and CG
`[6, 39.5]` %MAC). This is a property of regulatory documents (FCOM /
operator limits), independent of the lookup data. Prior to PR
`fix/envelope-bounds-and-menu-order` (2026-05-19) the bounds were
estimated conservatively as `[110, 172]` / `[8, 35]` for MVP scoping;
the FCOM-certified envelope is wider in all four directions.
...
| `operationalEnvelope.weight` | { minTons, maxTons } | Регуляторный диапазон веса. Алгоритм этот диапазон не проверяет — проверка делается use-case-функцией `validateWeightEnvelope` (см. crosswind contract). За границами envelope UI показывает field error + warning chip рядом с числом. |
| `operationalEnvelope.cg` | { minPercent, maxPercent } | Регуляторный диапазон CG. Use-case-функция `validateCGEnvelope`; те же правила, что для веса. Weight и CG проверяются независимо, обе ошибки могут отображаться одновременно. |

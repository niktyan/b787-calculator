...
| Поле | Тип | Назначение |
|------|-----|------------|
| `schemaVersion` | string (semver) | Версия структуры файла. Структурный rebrand `1.x → 2.0.0` (Block 2). |
...
| `byAircraft.<aircraft>.<condition>.interpolation` | object | `model`, `slope`, `breakpoints[5]`. Один Excel-equivalent набор на каждое (aircraft × condition) — таблица отдельная. |
| `byAircraft.<aircraft>.<condition>.fallback` | object | `belowEnvelope: 'max-crosswind-40'` / `aboveEnvelope: 'ifna-fallback-40-match-excel'`. |
...
3. Для каждого присутствующего `byAircraft.<aircraft>.<condition>`:
   - `interpolation.slope !== 0` — иначе `CorruptedDataBundle`.
   - `interpolation.breakpoints` отсортированы по убыванию `crosswindKnots` (40, 35, 30, 25, 20) — иначе `CorruptedDataBundle`.
   - `interpolation.breakpoints` отсортированы по возрастанию `intercept` — иначе `CorruptedDataBundle`.

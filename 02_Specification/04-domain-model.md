...
2 ряда по 3 на compact). В **MVP активны `dry` (RWYCC 6), `good`
(RWYCC 5) и `mediumToGood` (RWYCC 4)**; остальные три значения
(`medium` / `mediumToPoor` / `poor`) валидны на уровне типов и UI
(видимы как disabled-сегменты с capability disclosure), но
соответствующие lookup-данные отсутствуют — выбор даёт
`DataNotAvailable.reason: 'condition-not-implemented'`. Активация
оставшихся трёх — PR 5/6/7.
...
содержит `byAircraft.b787_8.dry`, `byAircraft.b787_8.good` и
`byAircraft.b787_8.mediumToGood`.
...
| `byAircraft.<aircraft>.<condition>.strategyType` | `StrategyType` | Discriminator: одна из 5 strategy variants. В MVP `'bracketedLinear'` валидируется реальной zod-схемой и активна для Dry + Good + MediumToGood; остальные 4 литерала есть в union, но их `params` отклоняется (PR 5/6/7/8 их активируют). |
...
| `byAircraft.<aircraft>.<condition>.params.brackets[]` | `{ crosswindKnots, intercept }` | ≥ 2 брекета, отсортированы по убыванию `crosswindKnots` (Dry: 40,35,30,25,20; Good: 40,35,30,25,20,15; MediumToGood: 35,30,25,20,15,10) и возрастанию `intercept`. |
| `byAircraft.<aircraft>.<condition>.params.slope` | number | Общий наклон threshold-линий по весу (kilolbs). Должен быть ≠ 0. Dry — `0.0576`; Good — `0.06`; MediumToGood — `0.0436`. |
| `byAircraft.<aircraft>.<condition>.params.maxCap` | number ⎮ null | Верхний clamp результата. `null` — без clamp; число — clamp применяется после ROUNDDOWN при `result > maxCap` (cap inclusive). Для Dry — `37` (PR 2, per FCOM Tab 2.29.2a + Excel G8 `IF(G7>37,37,G7)`). Для Good — `37` (PR 3, shared с Dry per FCOM Tab 2.29.2a). Для MediumToGood — `null` (PR 4, Excel sheet не содержит G8). Medium/MediumToPoor/Poor — TBD (ожидается смесь). |
...
// Active strategy — bracketedLinear (Dry / PR 1; Good / PR 3; MediumToGood / PR 4)

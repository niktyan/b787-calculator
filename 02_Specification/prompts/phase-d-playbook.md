...
> **Status:** 🔄 **In progress** — `PRIVACY_POLICY.md` и
> `TERMS_OF_USE.md` опубликованы в корне репозитория через PR #78
> (branch `docs/phase-d-section-2-privacy-terms-hosting`) с
> разрешёнными плейсхолдерами: support email →
> `supportb787calculator@protonmail.com`; effective + last-updated
> dates → `19 May 2026`. После merge `deploy-pages.yml` автоматически
> деплоит HTML-копии на GitHub Pages. **Остаётся:** (1) подтвердить
> 200 OK на двух live URL после первого workflow-run; (2) отдельным
> follow-up PR заменить `<github-username>` placeholder в
> `PRIVACY_POLICY_URL` / `TERMS_OF_USE_URL` и `SUPPORT_EMAIL` в
> `src/core/constants.ts` на реальные значения.
...
- [ ] **§ 2** Privacy Policy + Terms of Use опубликованы на стабильных
      URL; `src/core/constants.ts` обновлён. (🔄 markdown-файлы
      замёрджены через PR #78; live URLs ждут первый Pages-deploy;
      обновление `constants.ts` — отдельный follow-up PR.)

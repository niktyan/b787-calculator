...
1. Возвращайтесь в браузер на странице PR.
2. Tab «Files changed» (опционально, бегло посмотрите diff — не для глубокого ревью, а просто чтобы видеть, какие файлы менялись).
3. Tab «Conversation», прокрутите вверх → кнопка зелёная «Review changes».
4. Выберите «Approve», в комментарии напишите:
   ```
   Functional review on iPhone XX through Expo Go — all checks pass. Approved.
   ```
5. Кнопка «Merge pull request» → выберите «Squash and merge» → подтвердите.
6. Ветка удалится автоматически.
7. Переходите к следующему промпту реализации.
